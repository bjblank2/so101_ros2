...
try:
    ...
    # Simplified version if deepdiff not available
    def DeepDiff(a, b):
        return a != b
try:
    ...
    # Simplified version if tqdm not available
    def progress_bar(items, desc):
        return items
    def progress_write(text):
        print(text)

from .utils import DeviceAlreadyConnectedError, DeviceNotConnectedError, enter_pressed
...
class MotorsBus(abc.ABC):
    ...
    def _has_different_ctrl_tables(self) -> bool:
        ...
        try:
            ...
            # Fallback to simple comparison if DeepDiff not available
            return any(
    ...
    def scan_port(cls, port: str, *args, **kwargs) -> dict[int, list[int]]:
        ...
        baudrate_ids = {}
        for baudrate in progress_bar(bus.available_baudrates, desc="Scanning port"):
            bus.set_baudrate(baudrate)
            ids_models = bus.broadcast_ping()
            if ids_models:
                progress_write(f"Motors found for {baudrate=}: {pformat(ids_models, indent=4)}")
    ...
    def set_half_turn_homings(self, motors: NameOrID | list[NameOrID] | None = None) -> dict[NameOrID, Value]:
        ...
        # Use individual reads since sync_read is not working reliably with Feetech motors
        actual_positions = {motor: self.read("Present_Position", motor, normalize=False) for motor in motors}
        ...
        # Longer delay after all EEPROM writes complete (EEPROM writes can take time)
        time.sleep(0.2)  # 200ms delay to allow all EEPROM writes to complete
        
        # Try reading positions again to verify they update correctly
        logger.debug("Re-reading positions after setting homing offsets...")
        for motor in motors:
            try:
                pos = self.read("Present_Position", motor, normalize=False, num_retry=2)
                logger.debug(f"  {motor}: position = {pos}")
            except Exception as e:
                logger.warning(f"  {motor}: Failed to read after setting offset: {e}")
    ...
    def record_ranges_of_motion(
        ...
        # Always use individual reads since sync_read is not working reliably with Feetech motors
        # sync_read succeeds but returns wrong values (2047/2048)
        start_positions = {motor: self.read("Present_Position", motor, normalize=False) for motor in motors}
        
        mins = start_positions.copy()
        maxes = start_positions.copy()

        # Use threading to handle input separately - avoids termios interference
        import threading
        user_pressed_enter = threading.Event()
        
        def wait_for_enter():
            """Thread function to wait for ENTER key in a separate thread."""
            try:
                input()  # Blocking input - this won't interfere with serial reads
        ...
        while not user_pressed_enter.is_set():
            ...
            for motor in motors:
                try:
                    ...
                    positions[motor] = maxes.get(motor, mins.get(motor, 2047))
            
            # Update min/max values
            for motor in motors:
                if motor in positions:
                    mins[motor] = min(positions[motor], mins[motor])
                    maxes[motor] = max(positions[motor], maxes[motor])

            # Display table rows - overwrite existing rows
            if display_values:
                # Calculate range for each motor
                ranges = {motor: maxes[motor] - mins[motor] for motor in motors}
                
                # Move cursor up to beginning of first data row
                for _ in range(len(motors)):
                    sys.stdout.write('\033[1A')  # Move up one line
                
                # Print each motor row, overwriting existing lines
                for motor in motors:
                    pos_val = positions.get(motor, mins.get(motor, 0))
                    range_val = ranges[motor]
                    status = "✓" if range_val > 0 else " "
                    row = f"{status} {motor:<15} | {pos_val:>8} | {mins[motor]:>6} | {maxes[motor]:>6} | {range_val:>6}"
                    sys.stdout.write('\r')       # Return to beginning of line
                    sys.stdout.write('\033[K')   # Clear from cursor to end of line
                    sys.stdout.write(row)        # Print the row
                    sys.stdout.write('\n')       # Move to next line
        ...
        # Show final results
        if display_values:
            print()  # One blank line after the table
        print("  Recording complete! Final ranges:")
        print(f"\n  {'Joint':<18} | {'Min':>6} | {'Max':>6} | {'Range':>6}")
        print("  " + "-" * 63)
        for motor in motors:
            range_val = maxes[motor] - mins[motor]
            status = "✓" if range_val > 0 else "⚠"
            print(f"  {status} {motor:<15} | {mins[motor]:>6} | {maxes[motor]:>6} | {range_val:>6}")
        print()
    ...
    def _sync_read(
        ...
        *,
        ...
        # Check if each motor actually responded before getting data
        values = {}
        unavailable_motors = []
        for id_ in motor_ids:
            if self.sync_reader.isAvailable(id_, addr, length):
                ...
                unavailable_motors.append(id_)
                if raise_on_error:
                    raise ConnectionError(
                        f"Motor ID {id_} did not respond to sync read. "
                        f"{err_msg}"
                    )
                logger.warning(f"Motor ID {id_} not available in sync read response")
        
        # If some motors didn't respond, sync_read partially failed
        if unavailable_motors and raise_on_error:
            raise ConnectionError(
                f"Motors {unavailable_motors} did not respond to sync read. {err_msg}"
            )
